...
export default function ReviewWorkbench() {
  ...
  const reviewedInteractions = interactions?.filter(
    (i) => i.reviews && i.reviews.length > 0
  ) || [];
          ...
          <div className="w-96 border-r border-border bg-muted/30">
            <div className="p-4 border-b border-border">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {pendingInteractions.length} pending • {reviewedInteractions.length} reviewed
              </p>
            </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="h-[calc(100vh-8rem)]">
              <TabsList className="grid w-full grid-cols-2">
                ...
                <TabsTrigger value="reviewed" data-testid="tab-reviewed">
                  Reviewed ({reviewedInteractions.length})
                </TabsTrigger>
              </TabsList>
              
              <TabsContent value="pending" className="h-[calc(100%-3rem)] m-0">
              ...
              <TabsContent value="reviewed" className="h-[calc(100%-3rem)] m-0">
                <ScrollArea className="h-full">
                  <div className="p-4 space-y-3">
                    {isLoading ? (
                      [...Array(3)].map((_, i) => (
                      ...
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        <ClipboardList className="w-12 h-12 text-muted-foreground mb-3" />
                        <p className="text-sm text-muted-foreground">No reviewed items</p>
                        <p className="text-xs text-muted-foreground">Completed reviews will appear here</p>
                      </div>
                    ) : (
                      reviewedInteractions.map((interaction) => (
                        <Card 
                          ...
                          onClick={() => setSelectedInteraction(interaction)}
                          data-testid={`reviewed-item-${interaction.id}`}
                        >
                          <CardContent className="p-4">
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex-1">
                              ...
                              {interaction.reviews && interaction.reviews[0] && (
                                <Badge 
                                  variant={
                                    interaction.reviews[0].reviewDecision === 'agree' ? 'default' :
                                    interaction.reviews[0].reviewDecision === 'agree_with_thoughts' ? 'secondary' :
                                    interaction.reviews[0].reviewDecision === 'disagree' ? 'outline' :
                                    'destructive'
                                  }
                                  className="text-xs"
                                >
                                  {interaction.reviews[0].reviewDecision.replace(/_/g, ' ')}
                                </Badge>
                              )}
                            </div>
                            ...
                            {interaction.reviews && interaction.reviews[0] && interaction.reviews[0].reviewedAt && (
                              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                <Clock className="w-3 h-3" />
                                <span>
                                  Reviewed {formatDistanceToNow(new Date(interaction.reviews[0].reviewedAt), { addSuffix: true })}
                                </span>
                              </div>
                            )}
                          </CardContent>
